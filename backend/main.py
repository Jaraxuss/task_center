...
@app.get("/api/review-batches/{batch_id}/customer-materials", response_model=list[CustomerMaterialV2Read])
def list_batch_materials(batch_id: int, db: Session = Depends(get_db)) -> list[CustomerMaterialV2Read]:
    batch = db.get(ReviewBatch, batch_id)
    if not batch:
        raise HTTPException(status_code=404, detail="Review batch not found")
    ...
    return [serialize_material_v2(m) for m in db.scalars(stmt).all()]
