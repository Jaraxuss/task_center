...
```text
backend/services/feishu_card/
├── __init__.py
├── card.py      # Card JSON 2.0 builder / Markdown 转卡片 / 大小校验
└── client.py    # tenant_access_token 缓存 + im/v1/messages 发送

backend/scripts/send_feishu_card.py  # SDK smoke/运维脚本：文本/Markdown/JSON → 卡片发送
```
...
from services.feishu_card import FeishuCardClient, FeishuTarget, text_to_card_v2
...
```python
from services.feishu_card import markdown_to_card_v2
...
```bash
# 纯文本 → 简单卡片，仅预览不发送
python backend/scripts/send_feishu_card.py --text "提醒：测试" --dry-run

# Markdown → Card V2 → 发送给默认接收者
python backend/scripts/send_feishu_card.py --markdown report.md --template green --split

# 已有 Card JSON → 发送给指定群
python backend/scripts/send_feishu_card.py --card card.json --to oc_xxx --type chat_id
```
