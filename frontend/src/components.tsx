    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-panel card">
          <span className="label-caption">当前工作模式</span>
...
export function SummaryCards({
  ...
  items: Array<{ label: string; value: number; tone?: 'neutral' | 'brand' | 'success' | 'danger'; helper?: string }>;
}) {
  return (
    <div className="summary-grid">
      {items.map((item) => (
        <div className={`card summary-card tone-${item.tone || 'neutral'}`} key={item.label}>
          <span className="label-caption">{item.label}</span>
...
export function TaskDetail({
  ...
  if (!task) {
    return <EmptyState title="选择一个任务" description="左边点一下，右边这块工作台才会真正开始干活。" />;
  }
  ...
  return (
    <aside className="detail-panel card card-elevated">
      ...
      <div className="detail-section-grid detail-section-grid-secondary">
        <div className="detail-section card section-card">
          <div className="detail-section-title">
            <h4>提醒记录</h4>
